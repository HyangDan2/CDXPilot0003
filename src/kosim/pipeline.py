...
class PipelineArtifacts:
    ...
    llm_report_path: Path | None
    chart_paths: list[Path]
    condition_results: list[ConditionRunArtifacts] = field(default_factory=list)
...
class DailyPipelineRunner:
    ...
    def run(self) -> PipelineArtifacts:
        ...
        run_tag = f"run_{dates[0].isoformat()}_{dates[-1].isoformat()}"
        run_dir = report_dir / run_tag
        run_dir.mkdir(parents=True, exist_ok=True)

        self.emit(RunEvent("markdown_report_generation", "running", "Generating markdown reports"))
        raw_path = run_dir / "raw_data.md"
        bridge_path = run_dir / "llm_bridge.md"
        sim_path = run_dir / "simulation_report.md"
        ...
        self.emit(RunEvent("markdown_report_generation", "success", f"Reports written to {run_dir}"))
        ...
        if telegram.enabled and chart_paths:
            telegram_queue = cfg.get("telegram", {}).get("queue", {})
            queue_only = bool(telegram_queue.get("send_only_llm_queue_items", True))
            if chart_cfg.get("send_summary_charts", True):
                for path in summary_chart_paths(chart_paths):
                    telegram.send_photo(path, path.stem)
            if chart_cfg.get("send_case_charts", True) and not queue_only:
                for path in case_chart_paths(chart_paths)[: int(chart_cfg.get("case_chart_send_limit", 20))]:
                    telegram.send_photo(path, path.stem)

        prompt_path: Path | None = None
        llm_report_path: Path | None = None
        condition_results = _build_condition_artifacts(cfg, result, run_dir)
        if bool(cfg.get("llm", {}).get("enabled", False)):
            llm_client = OpenAICompatibleLLMClient(cfg["llm"])
            queue_cfg = cfg.get("llm", {}).get("queue", {})
            max_items = int(queue_cfg.get("max_items", 6))
            include_empty = bool(queue_cfg.get("include_empty_results", False))
            queue_items = [
                item
                for item in condition_results
                if _condition_llm_queue(cfg, item.condition_name) and (include_empty or item.result.trades)
            ][:max_items]
            if not bool(queue_cfg.get("enabled", True)):
                queue_items = []
            for item in queue_items:
                self.emit(RunEvent("llm_prompt_build", "running", f"Building stateless LLM prompt for {item.condition_name}"))
                prompt = build_llm_prompt(
                    _condition_only_config(cfg, item.condition_name),
                    item.result,
                    data_limitations=data_limitations or ["No known limitations reported by pipeline."],
                    condition_name=item.condition_name,
                )
                prompt_path = item.report_dir / "llm_prompt.txt"
                prompt_path.write_text(prompt, encoding="utf-8")
                item.llm_prompt_path = prompt_path
                self.emit(RunEvent("llm_prompt_build", "success", f"LLM prompt written for {item.condition_name}"))

                self.emit(RunEvent("llm_analysis", "running", f"Calling LLM for {item.condition_name}"))
                try:
                    llm_response = llm_client.analyze(prompt)
                except Exception as exc:
                    self.emit(RunEvent("llm_analysis", "warning", f"LLM failed for {item.condition_name}: {exc}"))
                    continue
                item.llm_report_path = item.report_dir / "llm_report.md"
                item.llm_report_path.write_text(llm_response.content, encoding="utf-8")
                llm_report_path = item.llm_report_path
                self.emit(RunEvent("llm_analysis", "success", f"LLM report generated for {item.condition_name} by {llm_response.model}"))

                self.emit(RunEvent("telegram_llm_report_send", "running", f"Sending LLM report for {item.condition_name}"))
                ...
                if cfg.get("telegram", {}).get("delivery", {}).get("send_llm_report_file", False):
                    telegram.send_file(item.llm_report_path, f"LLM strategy report: {item.condition_name}")
                telegram_queue = cfg.get("telegram", {}).get("queue", {})
                if telegram.enabled and telegram_queue.get("send_condition_charts", True):
                    for chart_path in item.chart_paths[: int(telegram_queue.get("max_condition_charts", 3))]:
                        telegram.send_photo(chart_path, f"{item.condition_name}: {chart_path.stem}")
                self.emit(RunEvent("telegram_llm_report_send", "success" if telegram.enabled else "skipped", f"LLM Telegram step finished for {item.condition_name}"))
        else:
        ...
        return PipelineArtifacts(result, raw_path, bridge_path, sim_path, prompt_path, llm_report_path, chart_paths, condition_results)

    def _provider(self):
        return create_market_data_provider(self.config.values)


def _build_condition_artifacts(config: dict, result: SimulationResult, run_dir: Path) -> list[ConditionRunArtifacts]:
    artifacts: list[ConditionRunArtifacts] = []
    condition_names = [case.get("name") for case in config.get("simulation", {}).get("strategy_conditions", []) if case.get("enabled", True)]
    for condition_name in condition_names:
        if not condition_name:
            continue
        condition_result = _filter_result_by_condition(result, condition_name)
        condition_dir = run_dir / "conditions" / _safe_name(condition_name)
        condition_dir.mkdir(parents=True, exist_ok=True)
        bridge_path = condition_dir / "llm_bridge.md"
        sim_path = condition_dir / "simulation_report.md"
        condition_cfg = _condition_only_config(config, condition_name)
        bridge_path.write_text(llm_bridge_markdown(condition_cfg, condition_result), encoding="utf-8")
        sim_path.write_text(
            simulation_report_markdown(condition_result, int(config.get("report", {}).get("markdown", {}).get("top_n_conditions", 20))),
            encoding="utf-8",
        )
        chart_paths = generate_charts(
            condition_result,
            condition_dir / "charts",
            int(config.get("telegram", {}).get("queue", {}).get("max_condition_charts", 3)),
        )
        artifacts.append(
            ConditionRunArtifacts(
                condition_name=condition_name,
                result=condition_result,
                report_dir=condition_dir,
                llm_bridge_path=bridge_path,
                simulation_report_path=sim_path,
...
def _filter_result_by_condition(result: SimulationResult, condition_name: str) -> SimulationResult:
    return SimulationResult(
        raw_data=result.raw_data,
        trades=[trade for trade in result.trades if trade.condition_name == condition_name],
        metrics=[metric for metric in result.metrics if metric.condition_name == condition_name],
    )


def _condition_llm_queue(config: dict, condition_name: str) -> bool:
    for case in config.get("simulation", {}).get("strategy_conditions", []):
        if case.get("name") == condition_name:
            return bool(case.get("llm_queue", False))
    return False


def _condition_only_config(config: dict, condition_name: str) -> dict:
    condition_cases = [
        case
        for case in config.get("simulation", {}).get("strategy_conditions", [])
        if case.get("name") == condition_name
    ...
    copied = dict(config)
    copied["simulation"] = dict(config.get("simulation", {}))
    copied["simulation"]["strategy_conditions"] = condition_cases
    return copied
